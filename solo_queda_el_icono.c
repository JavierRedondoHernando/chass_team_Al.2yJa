...
void ajedrez(struct datos_partidas *datos, int cargar);
...
void main() {
	...
	int codigo_peticion;
	int i, j, k, booleana, num_jugador, num_partida, tiempo_inicio, n_jugadores_en_fichero, n_partidas_en_fichero;
	...
	do {
		...
		while (fscanf(partidas, "%s %s %s %s", vect_partidas[i].titulo, vect_partidas[i].Nombre_jugador1,
			...
			for (k = 0; k < DIM; k++) {
				fscanf(partidas, "%d %d %d %d %d %d %d %d %d %d", &vect_partidas[i].tablero_piezas[k][0],
					&vect_partidas[i].tablero_piezas[k][1], &vect_partidas[i].tablero_piezas[k][2],
					&vect_partidas[i].tablero_piezas[k][3], &vect_partidas[i].tablero_piezas[k][4],
					&vect_partidas[i].tablero_piezas[k][5], &vect_partidas[i].tablero_piezas[k][6],
					&vect_partidas[i].tablero_piezas[k][7], &vect_partidas[i].tablero_piezas[k][8],
					&vect_partidas[i].tablero_piezas[k][9]);
			}
			i++;
		}
		fclose(partidas);
		n_partidas_en_fichero = i;
		...
		case 11://-----------------------------------------------------
			system("Cls()");
			for (k = 0; k < 2; k++, n_jugadores_en_fichero++) {
				do {
					printf("\n\tPARTIDA NUEVA\n");
					printf("\n> Nuevos jugadores");
					...
					for (j = 0; j < n_jugadores_en_fichero; j++) {
				...
				jugadores[n_jugadores_en_fichero] = jugadores_en_partida[k];
			...
			ajedrez(&vect_partidas[n_partidas_en_fichero], 0);
			...
			fclose(datos);
			printf("\n");
			if (confirmacion("guardar la partida actual") == 1) {
				do {
					system("cls()");
					printf("\n>GUARDAR PARTIDA\n\n\nNombre de la partida: ");
					gets(vect_partidas[n_partidas_en_fichero].titulo);
					for (j = 0; j < LONGNOMBRES; j++) {
						if (vect_partidas[n_partidas_en_fichero].titulo[j] == ' ') {
							vect_partidas[n_partidas_en_fichero].titulo[j] = '_';
						}
						if (j == LONGNOMBRES - 1) {
							vect_partidas[n_partidas_en_fichero].titulo[j] = '\0';
						}
					}

					for (j = 0; j < n_partidas_en_fichero; j++) {
						if (strcmp(vect_partidas[n_partidas_en_fichero].titulo, vect_partidas[j].titulo) == 0) {
							booleana = 0;
							break;
						}
						else {
					...
					if (booleana == 0) {
						printf("\n\nEl nombre de la partida que has introducido ya existe\n\n");
						system("Pause()");
						system("cls");
					}
				} while (booleana == 0);
				strcpy(vect_partidas[n_partidas_en_fichero].Nombre_jugador1, jugadores_en_partida[0].nombre);
				strcpy(vect_partidas[n_partidas_en_fichero].Nombre_jugador2, jugadores_en_partida[1].nombre);
				funcionfecha(vect_partidas[n_partidas_en_fichero].fecha_ultima);				
				printf("\n\nLa partida se ha guardado con el siguiente nombre:\n\n >> ");
				printf("%s %s vs %s %s\n\n", vect_partidas[n_partidas_en_fichero].titulo,
					vect_partidas[n_partidas_en_fichero].Nombre_jugador1,
					vect_partidas[n_partidas_en_fichero].Nombre_jugador2,
					vect_partidas[n_partidas_en_fichero].fecha_ultima);

				i = 0;
				fopen_s(&partidas, "memoria_partidas_guardadas.txt", "w");
				for (i = 0; i < n_partidas_en_fichero + 1; i++) {
					fprintf(partidas, "%s %s %s %s\n", vect_partidas[i].titulo, vect_partidas[i].Nombre_jugador1,
						vect_partidas[i].Nombre_jugador2, vect_partidas[i].fecha_ultima);
					for (k = 0; k < DIM; k++) {
						fprintf(partidas, "%d %d %d %d %d %d %d %d %d %d\n", vect_partidas[i].tablero_piezas[k][0],
							vect_partidas[i].tablero_piezas[k][1], vect_partidas[i].tablero_piezas[k][2],
							vect_partidas[i].tablero_piezas[k][3], vect_partidas[i].tablero_piezas[k][4],
							vect_partidas[i].tablero_piezas[k][5], vect_partidas[i].tablero_piezas[k][6],
							vect_partidas[i].tablero_piezas[k][7], vect_partidas[i].tablero_piezas[k][8],
		...
		case 12://-----------------------------------------------------
			...
			if (n_jugadores_en_fichero < 2) {
				...
				for (j = 0; j < n_jugadores_en_fichero; j++) {
					for (k = j + 1; k < n_jugadores_en_fichero; k++) {
					...
					for (k = 0; k < 2; k++) {
						if (booleana == 1) {
							while (1) {
								...
								scanf("%d", &num_jugador);
								getchar();
								if ((num_jugador > 0) && (num_jugador < (n_jugadores_en_fichero + 1))) {
									system("Cls()");
		...
		case 20://-----------------------------------------------------
			...
			if (n_partidas_en_fichero== 0) {
				...
				while (1) {
					...
					scanf("%d", &num_partida);
					if ((num_partida > 0) && (num_partida < (n_partidas_en_fichero + 1))) {
						system("Cls()");
						printf("\n\tPARTIDA GUARDADA\n\n >> ");
						...
						for (k = 0; k < n_jugadores_en_fichero; k++) {
							if (strcmp(vect_partidas[num_partida - 1].Nombre_jugador1, jugadores[k].nombre) == 0) {
								break;
							}
						}
						for (j = 0; j < n_jugadores_en_fichero; j++) {
							if (strcmp(vect_partidas[num_partida - 1].Nombre_jugador2, jugadores[j].nombre) == 0) {
								break;
							}
						}

						if ((k==n_jugadores_en_fichero)||(j==n_jugadores_en_fichero)) {//No ha encontrado los dos jugadores, porque se habrán borrado anteriormente
							printf("\n\n\nAlguno de los jugadores no existe o ha sido borrado anteriormente. \nEsta partida ya no se puede jugar.");
							printf(" Deberia eliminar esta partida guardada\n\n");
							system("Pause()");
							break;
						}
						else {
							...
							printf("\n\tPARTIDA GUARDADA\n\n >> ");
							printf("%s\n", vect_partidas[num_partida - 1].Nombre_jugador1);
							if (verif_password(jugadores[k].password) == 0) {
								if (confirmacion("volver a intentarlo") == 1) {
									continue;
								}
								else {
							...
							printf("\n\tPARTIDA GUARDADA\n\n >> ");
							printf("%s\n", vect_partidas[num_partida - 1].Nombre_jugador2);
							if (verif_password(jugadores[j].password) == 0) {
								if (confirmacion("volver a intentarlo") == 1) {
									continue;
								}
								else {
							...
							printf("Datos correctos. Comienza la partida\n\n");
							system("Pause()");
							Sleep(100);
							tiempo_inicio = tiempo_actual_a_minutos();
							ajedrez(&vect_partidas[num_partida-1], 1);

							jugadores[k].minutos_jugados = jugadores[k].minutos_jugados + (tiempo_actual_a_minutos() - tiempo_inicio);
							jugadores[j].minutos_jugados = jugadores[j].minutos_jugados + (tiempo_actual_a_minutos() - tiempo_inicio);
							jugadores[k].partidas_jugadas++;
							jugadores[j].partidas_jugadas++;
							funcionfecha(vect_partidas[num_partida - 1].fecha_ultima);
							...
							for (j = 0; j < n_partidas_en_fichero; j++) {
								...
								for (k = 0; k < DIM; k++) {
									fprintf(partidas, "%d %d %d %d %d %d %d %d %d %d\n", vect_partidas[j].tablero_piezas[k][0],
										vect_partidas[j].tablero_piezas[k][1], vect_partidas[j].tablero_piezas[k][2],
										vect_partidas[j].tablero_piezas[k][3], vect_partidas[j].tablero_piezas[k][4],
										vect_partidas[j].tablero_piezas[k][5], vect_partidas[j].tablero_piezas[k][6],
										vect_partidas[j].tablero_piezas[k][7], vect_partidas[j].tablero_piezas[k][8],
										vect_partidas[j].tablero_piezas[k][9]);
								}
		...
		case 412://----------------------------------------------------
			...
			fopen_s(&datos, "memoria_datos.txt", "w");
			for (j = 0; j < (n_jugadores_en_fichero + 1); j++) {
				fprintf(datos, "%s %s %d %d\n", jugadores[j].nombre, jugadores[j].password,
					jugadores[j].partidas_jugadas, jugadores[j].minutos_jugados);
			}
			fclose(datos);
		...
		case 421://----------------------------------------------------
			...
			if (n_partidas_en_fichero == 0) {
				...
				for (i = 0; i < n_partidas_en_fichero; i++) {
					printf("> ");
					for (k = 0; k < LONGNOMBRES; k++) {
		...
		case 423://----------------------------------------------------
			...
			printf("\nNo se ha eliminado ninguan partida\n\n");
			system("Pause()");
			break;
		case 431://----------------------------------------------------
			do {
...
void ajedrez(struct datos_partidas *datos, int cargar) {
	...
	if (cargar == 0) {
		//INICIALIZAR EL TABLERO CON LAS PIEZAS  en caso de que sea una partida nueva

		//INICIALIZA EL TABLERO DE PIEZAS
		for (fila = 0; fila < DIM; fila++) {
			for (columna = 0; columna < DIM; columna++) {
				datos->tablero_piezas[fila][columna] = 0;
			}
		}
		...
		for (i = 0; i < MAX_PIEZAS; i++) {
			if (i < 8) {
				...
				datos->tablero_piezas[piezas.peon[i].fila][piezas.peon[i].columna] = piezas.peon[i].valor;
			}
			else {
			...
			if (i < 2) {
				...
				datos->tablero_piezas[piezas.alfil[i].fila][piezas.alfil[i].columna] = piezas.alfil[i].valor;
				datos->tablero_piezas[piezas.caballo[i].fila][piezas.caballo[i].columna] = piezas.caballo[i].valor;
				datos->tablero_piezas[piezas.torre[i].fila][piezas.torre[i].columna] = piezas.torre[i].valor;
			}
			else {
				piezas.caballo[i].valor = 2;
		...
		for (i = 0; i < MAX_PIEZAS; i++) {
			if (i == 0) {
				...
				datos->tablero_piezas[piezas.reina[i].fila][piezas.reina[i].columna] = piezas.reina[i].valor;
			}
			else {
		...
		datos->tablero_piezas[piezas.rey.fila][piezas.rey.columna] = piezas.rey.valor;
		...
		for (i = 0; i < 8; i++) {
			...
			datos->tablero_piezas[piezas.peon_n[i].fila][piezas.peon_n[i].columna] = piezas.peon_n[i].valor;
		}
		for (i = 0; i < 2; i++) {
			...
			datos->tablero_piezas[piezas.alfil_n[i].fila][piezas.alfil_n[i].columna] = piezas.alfil_n[i].valor;
			datos->tablero_piezas[piezas.caballo_n[i].fila][piezas.caballo_n[i].columna] = piezas.caballo_n[i].valor;
			datos->tablero_piezas[piezas.torre_n[i].fila][piezas.torre_n[i].columna] = piezas.torre_n[i].valor;
		}
		for (i = 0; i < MAX_PIEZAS; i++) {
			if (i == 0) {
				...
				datos->tablero_piezas[piezas.reina_n[i].fila][piezas.reina_n[i].columna] = piezas.reina_n[i].valor;
			}
			else {
		...
		datos->tablero_piezas[piezas.rey_n.fila][piezas.rey_n.columna] = piezas.rey_n.valor;
	}
	else {
		//INICIALIZAR EL TABLERO CON LAS PIEZAS  en caso de que sea una partida cargada
		for (i = 0; i < MAX_PIEZAS; i++) {
			//PIEZAS BLANCAS
			piezas.peon[i].capturada = 1;
			piezas.peon[i].valor = 1;
			...
			piezas.rey.capturada = 1;
			piezas.rey.valor = 5;


			//PIEZAS NEGRAS
			piezas.peon_n[i].capturada = 1;
			piezas.peon_n[i].valor = 7;

			piezas.caballo_n[i].valor = 8;
			piezas.caballo_n[i].capturada = 1;

			piezas.alfil_n[i].valor = 9;
			piezas.alfil_n[i].capturada = 1;

			piezas.torre_n[i].valor = 10;
			piezas.torre_n[i].capturada = 1;

			piezas.reina_n[i].capturada = 1;
			piezas.reina_n[i].valor = 12;

			piezas.rey_n.capturada = 1;
			piezas.rey_n.valor = 11;
		}
		for (fila = 0; fila < DIM; fila++) {
			for (columna = 0; columna < DIM; columna++) {
				if (datos->tablero_piezas[fila][columna] == piezas.peon[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.peon[i].capturada == 1) {
							piezas.peon[i].capturada = 0;
							piezas.peon[i].fila = fila;
							piezas.peon[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.caballo[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.caballo[i].capturada == 1) {
							piezas.caballo[i].capturada = 0;
							piezas.caballo[i].fila = fila;
							piezas.caballo[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.alfil[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.alfil[i].capturada == 1) {
							piezas.alfil[i].capturada = 0;
							piezas.alfil[i].fila = fila;
							piezas.alfil[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.torre[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.torre[i].capturada == 1) {
							piezas.torre[i].capturada = 0;
							piezas.torre[i].fila = fila;
							piezas.torre[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.reina[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.reina[i].capturada == 1) {
							piezas.reina[i].capturada = 0;
							piezas.reina[i].fila = fila;
							piezas.reina[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.rey.valor) {
					piezas.rey.capturada = 0;
					piezas.rey.fila = fila;
					piezas.rey.columna = columna;
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.peon_n[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.peon_n[i].capturada == 1) {
							piezas.peon_n[i].capturada = 0;
							piezas.peon_n[i].fila = fila;
							piezas.peon_n[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.alfil_n[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.alfil_n[i].capturada == 1) {
							piezas.alfil_n[i].capturada = 0;
							piezas.alfil_n[i].fila = fila;
							piezas.alfil_n[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.caballo_n[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.caballo_n[i].capturada == 1) {
							piezas.caballo_n[i].capturada = 0;
							piezas.caballo_n[i].fila = fila;
							piezas.caballo_n[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.torre_n[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.torre_n[i].capturada == 1) {
							piezas.torre_n[i].capturada = 0;
							piezas.torre_n[i].fila = fila;
							piezas.torre_n[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.reina_n[0].valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.reina_n[i].capturada == 1) {
							piezas.reina_n[i].capturada = 0;
							piezas.reina_n[i].fila = fila;
							piezas.reina_n[i].columna = columna;
							break;
						}
					}
				}
				else if (datos->tablero_piezas[fila][columna] == piezas.rey_n.valor) {
					for (i = 0; i < MAX_PIEZAS; i++) {
							piezas.rey_n.capturada = 0;
							piezas.rey_n.fila = fila;
							piezas.rey_n.columna = columna;
							break;
					}
				}
			}
	...
	do {
		...
		if (contador_mov % 2 == 0) {
			...
			while (mov_producido == 0) {
				...
				imprimir_tablero(tablero_color, datos->tablero_piezas);
				jaque = check(datos->tablero_piezas, &piezas, contador_mov);
				if (jaque == 1) {
					printf("CHECK \n");
				}
				printf("Elija pieza: ");
				...
				case 'x':
				case 'X':
					return;//solo se permitaa la opcion de salir a las blandas
				case 'p':
				case 'P':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.peon[i].capturada == 0) {
							piezas.peon[i].revisar = revision_peon(datos->tablero_piezas, piezas.peon[i].fila, piezas.peon[i].columna, fila_aux, columna_aux);
					...
					if (num_piezas > 1) {
						...
						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.peon[i], fila_aux, columna_aux);
				...
				case'C':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.caballo[i].capturada == 0) {
							piezas.caballo[i].revisar = revision_caballo(datos->tablero_piezas, piezas.caballo[i].fila, piezas.caballo[i].columna, fila_aux, columna_aux, piezas.caballo[i].valor);
					...
					if (num_piezas > 1) {
						...
						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.caballo[i], fila_aux, columna_aux);
				...
				case 'A':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.alfil[i].capturada == 0) {
							piezas.alfil[i].revisar = revision_alfil(datos->tablero_piezas, piezas.alfil[i].fila, piezas.alfil[i].columna, fila_aux, columna_aux, piezas.alfil[i].valor);
					...
					if (num_piezas == 1) {
						for (i = 0; i < MAX_PIEZAS; i++) {
							if (piezas.alfil[i].revisar == 1) {
								...
								pieza_capturada(datos->tablero_piezas, &piezas, &piezas.alfil[i], fila_aux, columna_aux);
				...
				case 'T':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.torre[i].capturada == 0) {
							piezas.torre[i].revisar = revision_torre(datos->tablero_piezas, piezas.torre[i].fila, piezas.torre[i].columna, fila_aux, columna_aux, piezas.torre[i].valor);
					...
					if (num_piezas > 1) {
						...
						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.torre[i], fila_aux, columna_aux);
				...
				case 'R':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.reina[i].capturada == 0) {
							piezas.reina[i].revisar = revision_reina(datos->tablero_piezas, piezas.reina[i].fila, piezas.reina[i].columna, fila_aux, columna_aux, piezas.reina[i].valor);
					...
					if (num_piezas == 1) {
						for (i = 0; i < MAX_PIEZAS; i++) {
							if (piezas.reina[i].revisar == 1) {
								...
								pieza_capturada(datos->tablero_piezas, &piezas, &piezas.reina[i], fila_aux, columna_aux);
				...
				case 'K':
					...
					pedir_destino(&columna_aux, &fila_aux);
					piezas.rey.revisar = revision_rey(datos->tablero_piezas, piezas.rey.fila, piezas.rey.columna, fila_aux, columna_aux, piezas.rey.valor);

					if (piezas.rey.revisar == 1) {

						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.rey, fila_aux, columna_aux);
			...
			while (mov_producido == 0) {
				...
				case 'P':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.peon_n[i].capturada == 0) {
							piezas.peon_n[i].revisar = revision_peon_n(datos->tablero_piezas, piezas.peon_n[i].fila, piezas.peon_n[i].columna, fila_aux, columna_aux);
					...
					if (num_piezas > 1) {
						...
						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.peon_n[i], fila_aux, columna_aux);
				...
				case'C':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.caballo_n[i].capturada == 0) {
							piezas.caballo_n[i].revisar = revision_caballo(datos->tablero_piezas, piezas.caballo_n[i].fila, piezas.caballo_n[i].columna, fila_aux, columna_aux, piezas.caballo_n[i].valor);
					...
					if (num_piezas > 1) {
						...
						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.caballo_n[i], fila_aux, columna_aux);
				...
				case 'A':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.alfil_n[i].capturada == 0) {
							piezas.alfil_n[i].revisar = revision_alfil(datos->tablero_piezas, piezas.alfil_n[i].fila, piezas.alfil_n[i].columna, fila_aux, columna_aux, piezas.alfil_n[i].valor);
					...
					if (num_piezas == 1) {
						...
						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.alfil_n[i], fila_aux, columna_aux);
				...
				case 'T':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.torre_n[i].capturada == 0) {
							piezas.torre_n[i].revisar = revision_torre(datos->tablero_piezas, piezas.torre_n[i].fila, piezas.torre_n[i].columna, fila_aux, columna_aux, piezas.torre_n[i].valor);
					...
					if (num_piezas > 1) {
						...
						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.torre_n[i], fila_aux, columna_aux);
				...
				case 'R':
					...
					for (i = 0; i < MAX_PIEZAS; i++) {
						if (piezas.reina_n[i].capturada == 0) {
							piezas.reina_n[i].revisar = revision_reina(datos->tablero_piezas, piezas.reina_n[i].fila, piezas.reina_n[i].columna, fila_aux, columna_aux, piezas.reina_n[i].valor);
					...
					if (num_piezas == 1) {
						for (i = 0; i < MAX_PIEZAS; i++) {
							if (piezas.reina_n[i].revisar == 1) {
								...
								pieza_capturada(datos->tablero_piezas, &piezas, &piezas.reina_n[i], fila_aux, columna_aux);
				...
				case 'K':
					...
					pedir_destino(&columna_aux, &fila_aux);
					piezas.rey_n.revisar = revision_rey(datos->tablero_piezas, piezas.rey_n.fila, piezas.rey_n.columna, fila_aux, columna_aux, piezas.rey_n.valor);

					if (piezas.rey_n.revisar == 1) {

						pieza_capturada(datos->tablero_piezas, &piezas, &piezas.rey_n, fila_aux, columna_aux);
		...
		coronar(&datos->tablero_piezas, &piezas);
		contador_mov++;
		if (piezas.rey.capturada == 1) {
			printf("GANAN NEGRAS!!\n");
			system("pause");
			system("cls");
			return;
		}
		else if (piezas.rey_n.capturada == 1) {
...
int revision_peon(int tablero[DIM][DIM], int fila, int columna, int fila_aux, int   columna_aux) {
	...
	else if ((((fila == fila_aux + 2) && (columna == columna_aux)) && (tablero[fila_aux][columna_aux] == 0)) && (fila == 7)) {
		return 1;
	}
...
int revision_peon_n(int tablero[DIM][DIM], int fila, int columna, int fila_aux, int columna_aux) {
	...
	else if (((fila == fila_aux - 2) && (columna == columna_aux) && (tablero[fila_aux][columna_aux] == 0)) && (fila == 2)) {
		return 1;
	}
		...
		if ((fila - columna) == (fila_aux - columna_aux)) {
			...
			if (columna > columna_aux) {
				...
				for (i = columna - 1; i > columna_aux; i--) {
					j = j - 1;  //NO QUIERO PERDER EL VALOR Y USO UN AUXILIAR
					if (tablero[j][i] != 0) {
						return 0;
					}
				}
...
void pieza_capturada(int tablero[DIM][DIM], struct todas *piezas, struct pieza *pieza, int fila_aux, int columna_aux) {//la primera struct corresponde a la pieza que es capturad a la pieza capturada y la segunda a la que se mueve
	...
	else if (tablero[fila_aux][columna_aux] == piezas->rey.valor) {
		piezas->rey.capturada = 1;
	}
	...
	else if (tablero[fila_aux][columna_aux] == piezas->rey_n.valor) {
		piezas->rey_n.capturada = 1;
	}
	else {
